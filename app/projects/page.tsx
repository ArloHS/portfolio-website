...
const projects: Project[] = [
  ...
  // Unifi Africa work projects. Only approved scorecard projects appear here.
  {
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      <p className="text-center max-w-2xl mx-auto mb-12 text-slate-600 dark:text-slate-400">
        A showcase of my technical projects spanning data science, machine learning, and computer science.
      </p>
      ...
      <Tabs defaultValue="featured" className="w-full mb-12">
        ...
        <TabsContent value="computerscience" className="mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
...
function ProjectCard({ project, featured = false }: { project: Project; featured?: boolean }) {
  return (
    <Card
      className={`overflow-hidden transition-all hover:shadow-lg ${featured ? "border-teal-500 dark:border-teal-700" : ""}`}
    >
      <div className={`${featured ? "md:grid md:grid-cols-2 gap-6" : ""}`}>
        <div className={`h-64 relative ${featured ? "md:h-full" : ""}`}>
        ...
        <CardContent className={`p-6 ${featured ? "md:p-8" : ""}`}>
          <h3 className={`${featured ? "text-2xl" : "text-xl"} font-bold mb-3`}>{project.title}</h3>
          ...
          <p className={`text-slate-600 dark:text-slate-400 ${featured ? "" : "line-clamp-4"}`}>
            {featured ? project.description : project.description.split(".").slice(0, 3).join(".") + "."}
          </p>
